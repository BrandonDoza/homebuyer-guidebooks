export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-white px-4 py-8">
      <h1 className="text-4xl font-bold text-center mb-10 text-blue-800" >HomeBuyerGuidebooks.com</h1>

      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-6 max-w-6xl mx-auto">
      {guides.map((guide) => (
  <a
    key={guide.title}
    href={`/brandondoza/${guide.slug}`}
    className="border rounded-lg overflow-hidden hover:shadow-lg transition"
  >
    <img src={guide.image} alt={guide.title} className="w-full h-auto object-cover" />
    <div className="p-4 text-center text-lg font-medium text-blue-800">{guide.title}</div>
  </a>
))}
      </div>
